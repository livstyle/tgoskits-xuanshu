...
impl VmArchPerCpuOps for VmxPerCpuState {
    ...
    fn hardware_enable(&mut self) -> AxResult {
        ...
        if vmx_basic.is_32bit_address {
            return ax_err!(Unsupported, "unsupported 32-bit VMX physical address width");
        }
        if !vmx_basic.io_exit_info {
            return ax_err!(Unsupported, "VMX lacks I/O exit instruction info");
        }
        if !vmx_basic.vmx_flex_controls {
            return ax_err!(Unsupported, "VMX lacks flexible controls");
        }
